...
function versionFromRecord(record) {
  return { id: record.id, templateId: record.template, versionNumber: Number(record.version_number || 0), content: parseJson(record.content, { type: 'doc', content: [] }), sampleData: parseJson(record.sample_data, {}), pageConfig: parseJson(record.page_config, null), formVersionId: record.form_version || null, cssArtifactId: record.css_artifact || null, isPublished: Boolean(record.is_published), status: record.status, createdAt: record.created_at || record.created || '' };
}
...
function translateError(error, message) {
  ...
  const detailMessage = details && typeof details === 'object'
    ...
    : error?.message;
  return new DocumentDataAccessError(code, detailMessage ? `${message} ${detailMessage}` : message, { cause: error, details: details || {} });
}
...
export function createPocketBaseDocumentRepository(client) {
  ...
  return {
    ...
    async createTemplateVersion(input) { try { return versionFromRecord(await versions.create({ template: input.templateId, version_number: input.versionNumber, content: input.content, sample_data: input.sampleData ?? {}, page_config: input.pageConfig ?? {}, form_version: input.formVersionId || '', css_artifact: input.cssArtifactId || '', is_published: Boolean(input.isPublished), status: input.status, created_at: input.createdAt })); } catch (error) { throw translateError(error, 'Unable to create the document template version.'); } },
    async updateTemplateVersion(id, input) { try { return versionFromRecord(await versions.update(id, { ...(input.isPublished !== undefined && { is_published: Boolean(input.isPublished) }), ...(input.status !== undefined && { status: input.status }), ...(input.cssArtifactId !== undefined && { css_artifact: input.cssArtifactId || '' }) })); } catch (error) { throw translateError(error, 'Unable to update the document template version.'); } },
