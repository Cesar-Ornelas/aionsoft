...
export function createDocumentService(repository, dependencies = {}) {
  ...
  return {
    ...
    generate: async (input) => {
      ...
      const resolvedValues = materializeAggregateValues(formVersion?.schema, input.values ?? {});
      const renderedHtml = renderDocumentHtml(templateVersion.content, resolvedValues, formVersion?.schema ?? { fields: [] }, templateVersion.pageConfig, await listResources(templateVersion.templateId), await listVariables());
      return repository.createDocument({
        ...
        status: 'generated',
        dataSnapshot: structuredClone(resolvedValues),
        renderedHtml,
        createdAt: now()
      });
    }
