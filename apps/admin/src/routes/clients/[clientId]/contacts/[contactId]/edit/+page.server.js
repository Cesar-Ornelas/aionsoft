...
export async function load({ params }) {
	try {
		const [client, contact] = await Promise.all([
			...
			getClientContactById(params.clientId, params.contactId)
		]);
...
export const actions = {
	default: async ({ params, request }) => {
		...
		try {
			const contact = await updateClientContact(params.clientId, params.contactId, input);
		...
		throw redirect(303, `/clients/${params.clientId}/overview?contact=updated`);
	}
};