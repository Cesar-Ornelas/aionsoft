...
<section class="space-y-5">
	<header class="flex flex-col gap-4 border-b border-slate-200/80 pb-5 sm:flex-row sm:items-end sm:justify-between">
		<div>
			...
			<h1 class="mt-2 text-3xl font-semibold tracking-tight text-slate-950 sm:text-4xl">Complex project delivery</h1>
			...
			<div class="relative">
				...
				{#if actionsOpen}
					...
					<div class="absolute right-0 top-[calc(100%+0.6rem)] z-20 w-52 rounded-[1.3rem] border border-slate-200 bg-white p-2 shadow-[0_32px_80px_-32px_rgba(15,23,42,0.55)] ring-1 ring-slate-950/5">
						<a href="/projects/export" class="flex items-center gap-3 rounded-[1rem] px-3 py-2.5 text-sm font-medium text-slate-700 transition hover:bg-slate-50 hover:text-slate-950" onclick={() => (actionsOpen = false)}>
						...
						<button type="button" class="flex w-full items-center gap-3 rounded-[1rem] px-3 py-2.5 text-left text-sm font-medium text-slate-700 transition hover:bg-slate-50 hover:text-slate-950" onclick={openImportDialog}>
	...
	{#if data.errorMessage || form?.message}
		<div class="rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700">{data.errorMessage || form?.message}</div>
	{/if}
					...
					{:else}
						{#each data.projects as project}
							<tr class="bg-white align-top">
								<td class="px-5 py-4">
									<a href={`/projects/${project.id}/overview`} class="font-semibold text-slate-950 transition hover:text-slate-700 hover:underline">{project.name}</a>
									<p class="mt-1 max-w-md text-sm text-slate-500">{project.description || 'No description'}</p>
								</td>
								<td class="px-5 py-4 text-slate-600">{formatDate(project.startAt)} - {formatDate(project.dueAt)}</td>
								<td class="px-5 py-4">
									<div class="flex min-w-40 items-center gap-3">
										...
										<span class="text-xs font-semibold text-slate-600">{project.summary.averageProgress}%</span>
									</div>
								</td>
								<td class="px-5 py-4 text-slate-600">{project.summary.activeTasks} active / {project.summary.totalTasks} total</td>
								...
								<td class="px-5 py-4 text-right"><a href={`/projects/${project.id}/overview`} class="rounded-full border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:border-slate-300 hover:text-slate-950">Open</a></td>
							</tr>
						{/each}
					{/if}
				</tbody>
					...
					<div class="border-b border-slate-200 px-5 py-5 sm:px-6">
						<div class="flex items-start justify-between gap-4">
							<div>
								...
								<h2 class="mt-2 text-2xl font-semibold tracking-tight text-slate-950">Create project</h2>
								<p class="mt-2 text-sm leading-6 text-slate-600">New projects start with default phases and task lists so planning can begin immediately.</p>
							</div>
		...
		<div class="fixed inset-0 z-40 flex items-center justify-center p-4 sm:p-6">
			...
			<div class="relative z-10 w-full max-w-xl rounded-[1.8rem] border border-white/80 bg-white p-6 shadow-[0_32px_80px_-42px_rgba(15,23,42,0.45)] sm:p-7">
				<div class="border-b border-slate-200/80 pb-4">
					...
					<h2 class="mt-2 text-2xl font-semibold tracking-tight text-slate-950">Import projects JSON</h2>
					<p class="mt-2 text-sm leading-7 text-slate-600">Imported projects are added as new records with their internal relationships preserved.</p>
				</div>
				<form method="POST" action="?/import" enctype="multipart/form-data" class="mt-5 space-y-5">
					<input type="file" name="file" accept="application/json,.json" class="block w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600 file:mr-3 file:rounded-full file:border-0 file:bg-slate-950 file:px-3 file:py-1.5 file:text-sm file:font-semibold file:text-white" />
					<div class="rounded-2xl border border-sky-100 bg-sky-50 px-4 py-3 text-sm text-sky-800">This import is additive and creates new project IDs.</div>
					<div class="flex justify-end gap-3"><button type="button" class="rounded-full border border-slate-200 px-4 py-2.5 text-sm font-semibold text-slate-700" onclick={() => (importDialogOpen = false)}>Cancel</button><button class="rounded-full bg-slate-950 px-5 py-2.5 text-sm font-semibold text-white">Import projects</button></div>
				</form>
			</div>
		</div>
	{/if}
