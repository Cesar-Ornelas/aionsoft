...
<section class="space-y-6">
	<header class="flex flex-col gap-4 border-b border-slate-200/80 pb-5 lg:flex-row lg:items-end lg:justify-between">
		<div>
			...
			<h1 class="mt-2 text-3xl font-semibold tracking-tight text-slate-950 sm:text-4xl">Jobs</h1>
	...
	<div class="rounded-[1.9rem] border border-slate-200 bg-white p-2">
		<div class="grid gap-2 sm:grid-cols-2">
			<a href={viewHref('events')} class={`rounded-[1.4rem] px-4 py-3 text-sm font-semibold transition ${currentView() === 'events' ? 'bg-slate-950 text-white shadow-[0_20px_50px_-32px_rgba(15,23,42,0.8)]' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-950'}`}>
				Events
			...
			<a href={viewHref('definitions')} class={`rounded-[1.4rem] px-4 py-3 text-sm font-semibold transition ${currentView() === 'definitions' ? 'bg-slate-950 text-white shadow-[0_20px_50px_-32px_rgba(15,23,42,0.8)]' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-950'}`}>
				Definitions
	...
	{#if currentView() === 'events'}
		<form method="GET" class="rounded-[1.9rem] border border-slate-200 bg-white p-5">
			...
			<div class="grid gap-4 lg:grid-cols-[minmax(0,1.2fr)_12rem_11rem_11rem_auto] lg:items-end">
				<div>
					...
					<select id="job-filter" name="job" class="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm text-slate-950 outline-none transition focus:border-sky-300">
				...
				<div>
					...
					<select id="status-filter" name="status" class="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm text-slate-950 outline-none transition focus:border-sky-300">
				...
				<div>
					...
					<input id="date-from-filter" name="dateFrom" type="date" value={data.filters.dateFrom} class="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm text-slate-950 outline-none transition focus:border-sky-300" />
				</div>

				<div>
					<label for="date-to-filter" class="block text-sm font-semibold text-slate-800">To</label>
					<input id="date-to-filter" name="dateTo" type="date" value={data.filters.dateTo} class="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm text-slate-950 outline-none transition focus:border-sky-300" />
				</div>
				...
				<div class="flex gap-3 lg:justify-end">
					...
					<a href={viewHref('events')} class="rounded-full border border-slate-200 px-5 py-3 text-sm font-semibold text-slate-700 transition hover:border-slate-300 hover:text-slate-950">
						Reset
					</a>
				</div>
			</div>
		...
		<div class="space-y-5">
			{#if selectedGroup()}
				<div class="rounded-[1.9rem] border border-slate-200 bg-white p-6">
					<div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
						<div>
							<p class="text-xs font-semibold uppercase tracking-[0.24em] text-slate-400">Selected job history</p>
							<h2 class="mt-2 text-2xl font-semibold tracking-tight text-slate-950">{selectedGroup().jobName}</h2>
							<p class="mt-2 text-sm text-slate-500">{selectedGroup().jobKey}</p>
						</div>
						<div class="grid grid-cols-2 gap-3 text-sm sm:grid-cols-4">
							<div class="rounded-2xl bg-slate-50 px-4 py-3">
								<p class="text-xs uppercase tracking-[0.2em] text-slate-400">Total</p>
								<p class="mt-2 text-lg font-semibold text-slate-950">{selectedGroup().totalRuns}</p>
							</div>
					...
					{#if canRerunSelectedGroup()}
						<form method="POST" action="?/rerun" class="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 pt-5">
							<div>
								<p class="text-sm font-semibold text-slate-900">Manual rerun</p>
								<p class="mt-1 text-sm text-slate-600">Run this registered job now and store the result under the same job history.</p>
							</div>
			...
			{#if data.groups.length === 0}
				<div class="rounded-[1.9rem] border border-dashed border-slate-300 bg-white px-6 py-12 text-center">
					<p class="text-lg font-semibold text-slate-950">No job executions yet</p>
			...
			{:else}
				<div class="grid gap-5 xl:grid-cols-[minmax(0,0.9fr)_minmax(0,1.3fr)]">
					<div class="rounded-[1.9rem] border border-slate-200 bg-white p-5">
						<div class="flex items-start justify-between gap-3">
							<div>
								<p class="text-lg font-semibold tracking-tight text-slate-950">Job events</p>
								<p class="mt-2 text-sm leading-7 text-slate-600">
									Browse instrumented jobs and inspect the run stream for each one.
								</p>
							</div>
							<span class="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600">{data.groups.length} jobs</span>
						</div>

						<div class="mt-5 space-y-3">
							{#each data.groups as group}
								<a href={eventHref(group.jobKey)} class={`block rounded-2xl border px-4 py-4 transition ${group.jobKey === data.selectedEventJobKey ? 'border-sky-300 bg-sky-50' : 'border-slate-200 hover:border-slate-300 hover:bg-slate-50'}`}>
									<div class="flex items-start justify-between gap-3">
										<div>
											<p class="font-semibold text-slate-950">{group.jobName}</p>
									...
									<p class="mt-3 text-sm text-slate-600">{group.totalRuns} runs · last started {formatDate(group.lastStartedAt)}</p>
								</a>
							{/each}
						</div>
					</div>
								...
								<tbody class="divide-y divide-slate-100">
									{#each data.runs as run}
										<tr class="align-top">
											...
											<td class="px-5 py-4 text-slate-600">{formatDate(run.startedAt)}</td>
											<td class="px-5 py-4 text-slate-600">{formatDate(run.finishedAt)}</td>
											<td class="px-5 py-4 text-slate-600">{formatDuration(run.durationMs)}</td>
											<td class="px-5 py-4">
												{#if run.errorMessage}
												...
												{:else}
													<p class="text-sm text-slate-500">No details recorded.</p>
												{/if}
											</td>
										</tr>
									{/each}
	...
	{:else}
		<div class="rounded-[1.9rem] border border-slate-200 bg-white p-5">
			<div class="flex flex-col gap-3 border-b border-slate-200 pb-5 sm:flex-row sm:items-end sm:justify-between">
				<div>
					<p class="text-lg font-semibold tracking-tight text-slate-950">Saved job definitions</p>
					<p class="mt-2 max-w-3xl text-sm leading-7 text-slate-600">
				...
				<span class="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600">{data.jobDefinitions.length} definitions</span>
			</div>
			...
			<div class="mt-5 space-y-3">
				...
				{:else}
					{#each data.jobDefinitions as definition}
						<div class={`rounded-2xl border px-4 py-4 transition ${definition.jobKey === data.selectedDefinitionKey ? 'border-sky-300 bg-sky-50' : 'border-slate-200 bg-white'}`}>
							<div class="flex items-start justify-between gap-3">
								<a href={definitionHref(definition.jobKey)} class="min-w-0 flex-1">
									<div class="flex flex-wrap items-start gap-3">
										<div class="min-w-0 flex-1">
											<p class="font-semibold text-slate-950">{definition.jobName}</p>
									...
									<p class="mt-3 text-sm text-slate-600">{definition.schedule} · {definition.command}</p>
	...
	{#if dialogDefinition()}
		<div class="fixed inset-0 z-40">
			...
			<div class="absolute inset-x-4 top-1/2 mx-auto w-auto max-w-4xl -translate-y-1/2 rounded-[2rem] border border-slate-200 bg-white shadow-[0_40px_120px_-40px_rgba(15,23,42,0.6)] sm:inset-x-8 lg:inset-x-16" in:fade={{ duration: 180 }} out:fade={{ duration: 140 }}>
				<div class="flex items-start justify-between gap-4 border-b border-slate-200 px-6 py-5 sm:px-8">
					<div>
						...
						<h2 class="mt-2 text-2xl font-semibold tracking-tight text-slate-950">{dialogDefinition().jobName}</h2>
						<p class="mt-2 text-sm text-slate-500">{dialogDefinition().jobKey}</p>
					</div>
					<button type="button" class="rounded-full border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:border-slate-300 hover:text-slate-950" onclick={closeDefinitionDialog}>
						Close
					</button>
				</div>
				...
				<div class="space-y-5 px-6 py-6 sm:px-8 sm:py-8">
					...
					<div class="flex flex-wrap items-center justify-end gap-3">
						<button type="button" class="rounded-full border border-slate-200 px-4 py-2.5 text-sm font-semibold text-slate-700 transition hover:border-slate-300 hover:text-slate-950" onclick={closeDefinitionDialog}>
							Cancel
						</button>
	...
	{#if openDefinitionActionsKey}
		...
		<div class="fixed z-20 w-48 rounded-[1.3rem] border border-slate-200 bg-white p-2 text-left shadow-[0_32px_80px_-32px_rgba(15,23,42,0.55)] ring-1 ring-slate-950/5" style={`top: ${openDefinitionActionsTop}px; left: ${openDefinitionActionsLeft}px;`}>
			<a href={editDefinitionDrawerHref(openDefinitionActionsKey)} class="flex items-center gap-3 rounded-[1rem] px-3 py-2.5 text-sm font-medium text-slate-700 transition hover:bg-slate-50 hover:text-slate-950" onclick={closeDefinitionActions}>
				<svg viewBox="0 0 20 20" fill="none" class="h-4 w-4" aria-hidden="true">
			...
			<button type="button" class="flex w-full items-center gap-3 rounded-[1rem] px-3 py-2.5 text-left text-sm font-medium text-slate-700 transition hover:bg-slate-50 hover:text-slate-950" onclick={() => openDefinitionDialog(openDefinitionActionsKey)}>
				<svg viewBox="0 0 20 20" fill="none" class="h-4 w-4" aria-hidden="true">
			...
			<button type="button" class="flex w-full items-center gap-3 rounded-[1rem] px-3 py-2.5 text-left text-sm font-medium text-rose-700 transition hover:bg-rose-50" onclick={() => openDeleteDefinitionDialog(openDefinitionActionsKey)}>
					<svg viewBox="0 0 20 20" fill="none" class="h-4 w-4" aria-hidden="true">
			...
			<div class="relative z-10 w-full max-w-xl rounded-[1.8rem] border border-white/80 bg-white p-6 shadow-[0_32px_80px_-42px_rgba(15,23,42,0.45)] sm:p-7">
				<div class="flex items-start justify-between gap-4 border-b border-slate-200/80 pb-4">
					<div>
						...
						<h2 class="mt-2 text-2xl font-semibold tracking-tight text-slate-950">Delete job definition</h2>
						<p class="mt-2 text-sm leading-7 text-slate-600">
				...
				<form method="POST" action="?/deleteDefinition" class="mt-5 space-y-5">
					...
					<div class="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4 text-sm text-slate-700">
						<p class="font-semibold text-slate-950">{deleteDefinition().jobName}</p>
					...
					<div class="border-b border-slate-200 px-5 py-5 sm:px-6">
						<div class="flex items-start justify-between gap-4">
							<div>
								...
								<h2 class="mt-2 text-2xl font-semibold tracking-tight text-slate-950">{editDefinitionDrawerOpen() ? 'Edit job definition' : 'New job definition'}</h2>
								<p class="mt-2 text-sm leading-7 text-slate-600">
							...
							<a href={closeDefinitionDrawerHref()} class="rounded-full border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:border-slate-300 hover:text-slate-950">
								Close
							</a>
						</div>
					</div>
					...
					<form method="POST" action="?/saveDefinition" class="flex min-h-0 flex-1 flex-col">
						...
						<div class="min-h-0 flex-1 space-y-5 overflow-y-auto px-5 py-5 sm:px-6">
							<div class="grid gap-5 md:grid-cols-2">
								<div>
									<label for="jobKey" class="block text-sm font-semibold text-slate-800">Job key</label>
									<input id="jobKey" name="jobKey" type="text" value={definitionValues().jobKey} class="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm text-slate-950 outline-none transition focus:border-sky-300" placeholder="task-maintenance.reconcile-tags" />
								...
								<div>
									<label for="jobName" class="block text-sm font-semibold text-slate-800">Job name</label>
									<input id="jobName" name="jobName" type="text" value={definitionValues().jobName} class="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm text-slate-950 outline-none transition focus:border-sky-300" placeholder="Task maintenance reconciliation" />
								...
								<div class="md:col-span-2">
									...
									<textarea id="description" name="description" rows="4" class="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm text-slate-950 outline-none transition focus:border-sky-300" placeholder="Describe what this job does.">{definitionValues().description}</textarea>
								</div>

								<div>
									<label for="labelName" class="block text-sm font-semibold text-slate-800">Ofelia label name</label>
									<input id="labelName" name="labelName" type="text" value={definitionValues().labelName} class="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm text-slate-950 outline-none transition focus:border-sky-300" placeholder="task-maintenance" />
								</div>

								<div>
									<label for="schedule" class="block text-sm font-semibold text-slate-800">Schedule</label>
									<select id="schedulePreset" class="mt-2 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-950 outline-none transition focus:border-sky-300" value={selectedCronPreset()?.value ?? ''} onchange={handleSchedulePresetChange}>
									...
									{#if selectedCronPreset()}
										<p class="mt-2 text-xs text-slate-500">{selectedCronPreset().description}</p>
									{:else}
										<p class="mt-2 text-xs text-slate-500">Choose a standard schedule or type a custom cron rule below.</p>
									{/if}
									<input id="schedule" name="schedule" type="text" bind:value={scheduleValue} class="mt-3 w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm text-slate-950 outline-none transition focus:border-sky-300" placeholder="0 * * * *" />
								...
								<div>
									<label for="containerName" class="block text-sm font-semibold text-slate-800">Container name</label>
									<input id="containerName" name="containerName" type="text" value={definitionValues().containerName} class="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm text-slate-950 outline-none transition focus:border-sky-300" placeholder="aionsoft-admin" />
								...
								<div class="md:col-span-2">
									<label for="command" class="block text-sm font-semibold text-slate-800">Command</label>
									<input id="command" name="command" type="text" value={definitionValues().command} class="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm text-slate-950 outline-none transition focus:border-sky-300" placeholder="bun run task:maintenance" />
								...
								<div class="md:col-span-2">
									<label class="inline-flex items-center gap-3 rounded-2xl border border-slate-200 px-4 py-3 text-sm text-slate-700">
										<input type="checkbox" name="noOverlap" checked={definitionValues().noOverlap} class="h-4 w-4 rounded border-slate-300 text-slate-950" />
						...
						<div class="flex flex-wrap items-center justify-end gap-3 border-t border-slate-200 px-5 py-4 sm:px-6">
							<a href={closeDefinitionDrawerHref()} class="rounded-full border border-slate-200 px-4 py-2.5 text-sm font-semibold text-slate-700 transition hover:border-slate-300 hover:text-slate-950">
								Cancel
							</a>
