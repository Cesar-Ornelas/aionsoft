					...
					{:else}
						{#each data.roles as role}
							<tr class="bg-white">
								...
								<td class="px-5 py-4 align-top text-slate-600">{role.description || 'No description'}</td>
								<td class="px-5 py-4 align-top text-slate-500">{formatDate(role.updatedAt ?? role.createdAt)}</td>
								<td class="px-5 py-4 text-right align-top">
									<a href={`/security/roles/${role.id}/edit`} class="rounded-full border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:border-slate-300 hover:text-slate-950">
										Edit
									</a>
								</td>
							</tr>
				...
				<section class="flex h-full flex-col bg-white">
					<div class="border-b border-slate-200 px-5 py-5 sm:px-6">
						<div>
							...
							<p class="mt-2 text-sm leading-6 text-slate-600">Define a role without leaving the roles list.</p>
						</div>
					</div>
