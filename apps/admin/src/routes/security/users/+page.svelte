					...
					{:else}
						{#each data.users as user}
							<tr class="bg-white dark:bg-slate-900/92">
								...
								<td class="px-5 py-4 align-top text-slate-600 dark:text-slate-400">{user.email ?? 'No email'}</td>
								<td class="px-5 py-4 align-top text-slate-500 dark:text-slate-400">{formatDate(user.updatedAt ?? user.createdAt)}</td>
								<td class="px-5 py-4 text-right align-top">
									<a href={`/security/users/${user.id}/edit`} class="rounded-full border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:border-slate-300 hover:text-slate-950 dark:border-white/10 dark:text-slate-200 dark:hover:border-white/20 dark:hover:text-white">
										Edit
									</a>
								</td>
							</tr>
				...
				<section class="flex h-full flex-col bg-white dark:bg-slate-950">
					<div class="border-b border-slate-200 px-5 py-5 dark:border-white/10 sm:px-6">
						<div>
							...
							<h2 class="mt-2 text-2xl font-semibold tracking-tight text-slate-950 dark:text-white">Create a user</h2>
							<p class="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-400">Create a Logto user with email authentication without leaving the users list.</p>
						</div>
					</div>
						...
						<div class="min-h-0 flex-1 space-y-5 overflow-y-auto px-5 py-5 sm:px-6">
							...
							<div class="grid gap-5 sm:grid-cols-2">
								...
								<div>
									...
									<input id="email" name="email" type="email" value={values().email ?? ''} class="mt-2 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-950 outline-none transition focus:border-teal-400 focus:bg-white dark:border-white/10 dark:bg-white/5 dark:text-slate-100 dark:focus:border-teal-500/40 dark:focus:bg-white/10" placeholder="taylor@aionsoft.io" />
							...
							<div>
								...
								<input id="password" name="password" type="password" class="mt-2 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-950 outline-none transition focus:border-teal-400 focus:bg-white dark:border-white/10 dark:bg-white/5 dark:text-slate-100 dark:focus:border-teal-500/40 dark:focus:bg-white/10" placeholder="Set a temporary or permanent password" />
