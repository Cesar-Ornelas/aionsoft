<script>
	...
	let bulkUpdateOpen = $state(form?.intent === 'bulkUpdate');
	let selectedBulkTaskIds = $state(Array.isArray(form?.values?.taskIds) ? [...form.values.taskIds] : []);
	...
	function selectedTaskIds() {
		return Array.isArray(bulkFormValues.taskIds) ? bulkFormValues.taskIds : [];
	}
