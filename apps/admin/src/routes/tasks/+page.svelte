...
<section class="space-y-5">
	<header class="flex flex-col gap-4 border-b border-slate-200/80 pb-5 sm:flex-row sm:items-end sm:justify-between">
		<div>
			...
			<h1 class="mt-2 text-3xl font-semibold tracking-tight text-slate-950 sm:text-4xl">Task dashboard</h1>
	...
	<section class="overflow-hidden rounded-[1.9rem] border border-slate-200 bg-[linear-gradient(135deg,#fff7f7,#ffffff_58%,#f8fafc)] shadow-[0_24px_60px_-48px_rgba(15,23,42,0.75)]">
		<div class="grid gap-6 p-5 xl:grid-cols-[minmax(0,1.2fr)_minmax(0,0.8fr)] xl:p-7">
			<div>
				...
				<h2 class="max-w-3xl text-3xl font-semibold tracking-tight text-slate-950 sm:text-4xl">
			...
			<div class="rounded-[1.7rem] border border-slate-200 bg-white/90 p-5">
				<div class="flex items-center justify-between">
					<div>
						<p class="text-sm font-semibold text-slate-950">Today's focus</p>
						<p class="mt-1 text-sm text-slate-500">{formatHeaderDate()}</p>
					</div>
	...
	<section class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
		<article class="rounded-[1.7rem] border border-slate-200 bg-white p-5">
			<p class="text-sm text-slate-500">Open tasks</p>
			<div class="mt-3 flex items-end justify-between gap-3">
				<strong class="text-4xl font-semibold tracking-tight text-slate-950">{dashboard().summary.openTasksCount}</strong>
				<span class="rounded-full bg-sky-50 px-2.5 py-1 text-xs font-semibold text-sky-700">{dashboard().summary.filteredCount} filtered</span>
			</div>
		</article>
		...
		<article class="rounded-[1.7rem] border border-slate-200 bg-white p-5">
			...
			<div class="mt-3 flex items-end justify-between gap-3">
				...
				<span class="rounded-full bg-rose-50 px-2.5 py-1 text-xs font-semibold text-rose-700">Needs action</span>
			</div>
		</article>

		<article class="rounded-[1.7rem] border border-slate-200 bg-white p-5">
			<p class="text-sm text-slate-500">Client-facing work</p>
			<div class="mt-3 flex items-end justify-between gap-3">
				<strong class="text-4xl font-semibold tracking-tight text-sky-600">{dashboard().summary.customerScopedCount}</strong>
				<span class="rounded-full bg-sky-50 px-2.5 py-1 text-xs font-semibold text-sky-700">Audience set</span>
			</div>
		</article>

		<article class="rounded-[1.7rem] border border-slate-200 bg-white p-5">
			<p class="text-sm text-slate-500">Completed this month</p>
			<div class="mt-3 flex items-end justify-between gap-3">
				<strong class="text-4xl font-semibold tracking-tight text-emerald-600">{dashboard().summary.completedThisMonthCount}</strong>
				<span class="rounded-full bg-emerald-50 px-2.5 py-1 text-xs font-semibold text-emerald-700">Closed work</span>
			</div>
		</article>
	</section>
	...
	<section class="grid gap-4 xl:grid-cols-[minmax(0,1.2fr)_minmax(0,0.8fr)]">
		<article class="rounded-[1.9rem] border border-slate-200 bg-white p-5">
			<div class="mb-5 flex items-center justify-between gap-3">
				<div>
					<h3 class="text-lg font-semibold tracking-tight text-slate-950">Due date forecast</h3>
					<p class="text-sm text-slate-500">Active tasks grouped by upcoming due window</p>
				</div>
				<span class="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600">{dashboard().summary.totalCount} total tasks</span>
			</div>
			<div class="flex h-72 items-end gap-3 rounded-[1.6rem] border border-slate-200 bg-slate-50/70 p-4">
				{#each dashboard().dueForecast as bucket}
					<div class="flex flex-1 flex-col items-center gap-2">
						...
						<div class="text-center">
							<p class="text-xs font-semibold text-slate-700">{bucket.count}</p>
							<p class="text-xs text-slate-400">{bucket.label}</p>
						</div>
					</div>
				{/each}
			</div>
		...
		<article class="rounded-[1.9rem] border border-slate-200 bg-white p-5">
			<h3 class="text-lg font-semibold tracking-tight text-slate-950">Status mix</h3>
			<p class="mb-5 text-sm text-slate-500">Current workload across task states</p>
			<div class="rounded-[1.6rem] border border-slate-200 bg-slate-50/70 p-5">
				<p class="text-sm font-medium text-slate-500">Tracked tasks</p>
				<p class="mt-2 text-4xl font-semibold tracking-tight text-slate-950">{dashboard().statusMix.total}</p>
				<div class="mt-5 space-y-4">
					{#each dashboard().statusMix.buckets as bucket}
						<div>
							<div class="mb-2 flex justify-between text-sm">
								<span class="text-slate-700">{bucket.label}</span>
								<span class="font-semibold text-slate-950">{bucket.count}</span>
							</div>
	...
	<section class="grid gap-4 xl:grid-cols-3">
		<article class="rounded-[1.9rem] border border-slate-200 bg-white p-5">
			<h3 class="text-lg font-semibold tracking-tight text-slate-950">Client workload</h3>
			<p class="mb-5 text-sm text-slate-500">Open tasks grouped by audience</p>
			<div class="space-y-4">
				{#if dashboard().clientWorkload.length === 0}
					<p class="text-sm text-slate-500">No active workload is available yet.</p>
				{:else}
					{#each dashboard().clientWorkload as workload, index}
						<div>
							<div class="mb-2 flex justify-between text-sm">
								<span class="text-slate-700">{workload.label}</span>
								<span class="font-semibold text-slate-950">{workload.count}</span>
							</div>
							<div class="h-3 overflow-hidden rounded-full bg-slate-200">
		...
		<article class="rounded-[1.9rem] border border-slate-200 bg-white p-5">
			<h3 class="text-lg font-semibold tracking-tight text-slate-950">Attention needed</h3>
			<p class="mb-5 text-sm text-slate-500">Signals that deserve a quick follow-up</p>
			<div class="space-y-3">
		...
		<article class="rounded-[1.9rem] border border-slate-200 bg-white p-5">
			<h3 class="text-lg font-semibold tracking-tight text-slate-950">Weekly throughput</h3>
			<p class="mb-5 text-sm text-slate-500">Created versus completed this week</p>
			<div class="space-y-4">
					...
					<div class="grid grid-cols-[3rem_1fr] items-center gap-3">
						...
						<div class="space-y-1">
							<div class="flex gap-1">
								...
								<div class="h-3 rounded bg-slate-200" style={`width: ${Math.max(0, 100 - day.completedPercentage)}%`}></div>
							</div>
							<div class="flex gap-1">
								<div class="h-3 rounded bg-slate-400" style={`width: ${day.createdPercentage}%`}></div>
								<div class="h-3 rounded bg-slate-200" style={`width: ${Math.max(0, 100 - day.createdPercentage)}%`}></div>
							</div>
	...
	<section class="rounded-[1.9rem] border border-slate-200 bg-white">
		<div class="border-b border-slate-200/80 px-6 py-5">
			...
			<p class="mt-2 text-sm text-slate-500">Most recently touched tasks in the current filtered view.</p>
		</div>

		<div class="divide-y divide-slate-100">
			{#if dashboard().recentActivity.length === 0}
				<div class="px-6 py-10 text-sm text-slate-500">No recent task activity matches the current filters.</div>
			{:else}
					...
					<div class="px-6 py-5">
						<div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
							<div class="min-w-0">
								...
								<p class="mt-2 text-sm text-slate-500">{task.audienceLabel} · {task.progressPercentage}% complete</p>
							</div>

							<div class="sm:text-right">
								<p class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Last activity</p>
								<p class="mt-2 text-sm text-slate-600">{formatDate(task.activityAt)}</p>
							</div>
						</div>
					</div>
				{/each}
